...
void SmartCalcMainView::on_button_clear_clicked() {
  ...
  if (ui->result->hasFocus()) {
    ...
    size_t len = str.size();
    if (len > 0) {
      if (std::count(op.begin(), op.end(), str[len - 1]) ||
          str[str.size() - 1] == ')') {
        str.erase(str.size() - 1);
      } else if (isdigit(str[str.size() - 1]) || str[str.size() - 1] == '.') {
        while (len > 0 &&
               (isdigit(str[str.size() - 1]) || str[str.size() - 1] == '.')) {
          str.erase(str.size() - 1);
          len = str.size();
        }
      } else {
        ...
        while (len > 0 &&
               (isalpha(str[str.size() - 1]) || str[str.size() - 1] == '^')) {
          str.erase(str.size() - 1);
          len = str.size();
        }
      }
...
void SmartCalcMainView::on_credit_clicked() {
  ...
  if (total_amount && term && rate && total_amount > 0 && term > 0 &&
    ...
    ui->total_payout->setText(
        QString::number(total_amount + overpayment, 'f', 3));
  }
}
...
void SmartCalcMainView::on_delete_replenishmen_2_clicked() {
  if (ui->verticalLayout_7->count() > 1) {
    QDynamicButton *button = qobject_cast<QDynamicButton *>(
        ui->verticalLayout_7->itemAt(ui->verticalLayout_7->count() - 1)
            ->widget());
    button->hide();
    delete button;
  }
...
void SmartCalcMainView::on_delete_withdrawal_2_clicked() {
  if (ui->verticalLayout_8->count() > 1) {
    QDynamicButton *button = qobject_cast<QDynamicButton *>(
        ui->verticalLayout_8->itemAt(ui->verticalLayout_8->count() - 1)
            ->widget());
    button->hide();
    delete button;
  }
