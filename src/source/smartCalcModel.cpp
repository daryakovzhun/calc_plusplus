...
bool SmartCalcModel::to_postfix() {
  ...
  for (size_t i = 0; i < len_str && !error; i++) {
    if (isdigit(expression_[i]) || expression_[i] == 'x' ||
...
bool SmartCalcModel::check_double_dot(std::string *str_number) {
  const auto count_dot =
      std::count(str_number->cbegin(), str_number->cend(), '.');
  return count_dot > 1;
}
  ...
  for (size_t i = 0; i < len_str && !error; i++) {
    if (ex_postfix_[i] == 'x') {
      ...
      if (st.empty()) {
        ...
        st.pop();
        if (ex_postfix_[i] == '@' || ex_postfix_[i] == '~' ||
            is_func(ex_postfix_[i])) {
